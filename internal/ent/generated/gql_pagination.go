...
// DocumentRevisionEdge is the edge representation of DocumentRevision.
type DocumentRevisionEdge struct {
	Node   *DocumentRevision `json:"node"`
	Cursor Cursor            `json:"cursor"`
}

// DocumentRevisionConnection is the connection containing edges to DocumentRevision.
type DocumentRevisionConnection struct {
	Edges      []*DocumentRevisionEdge `json:"edges"`
	PageInfo   PageInfo                `json:"pageInfo"`
	TotalCount int                     `json:"totalCount"`
}

func (c *DocumentRevisionConnection) build(nodes []*DocumentRevision, pager *documentrevisionPager, after *Cursor, first *int, before *Cursor, last *int) {
	c.PageInfo.HasNextPage = before != nil
	c.PageInfo.HasPreviousPage = after != nil
	if first != nil && *first+1 == len(nodes) {
		c.PageInfo.HasNextPage = true
	...
	var nodeAt func(int) *DocumentRevision
	if last != nil {
		n := len(nodes) - 1
		nodeAt = func(i int) *DocumentRevision {
			return nodes[n-i]
		}
	} else {
		nodeAt = func(i int) *DocumentRevision {
	...
	c.Edges = make([]*DocumentRevisionEdge, len(nodes))
	for i := range nodes {
		node := nodeAt(i)
		c.Edges[i] = &DocumentRevisionEdge{
			Node:   node,
			Cursor: pager.toCursor(node),
		}
	}
...
// DocumentRevisionPaginateOption enables pagination customization.
type DocumentRevisionPaginateOption func(*documentrevisionPager) error

// WithDocumentRevisionOrder configures pagination ordering.
func WithDocumentRevisionOrder(order []*DocumentRevisionOrder) DocumentRevisionPaginateOption {
	return func(pager *documentrevisionPager) error {
		for _, o := range order {
			if err := o.Direction.Validate(); err != nil {
				return err
			}
...
// WithDocumentRevisionFilter configures pagination filter.
func WithDocumentRevisionFilter(filter func(*DocumentRevisionQuery) (*DocumentRevisionQuery, error)) DocumentRevisionPaginateOption {
	return func(pager *documentrevisionPager) error {
		if filter == nil {
			return errors.New("DocumentRevisionQuery filter cannot be nil")
		}
		pager.filter = filter
		return nil
	}
...
type documentrevisionPager struct {
	reverse bool
	order   []*DocumentRevisionOrder
	filter  func(*DocumentRevisionQuery) (*DocumentRevisionQuery, error)
}

func newDocumentRevisionPager(opts []DocumentRevisionPaginateOption, reverse bool) (*documentrevisionPager, error) {
	pager := &documentrevisionPager{reverse: reverse}
	for _, opt := range opts {
		if err := opt(pager); err != nil {
			return nil, err
		}
...
func (p *documentrevisionPager) applyFilter(query *DocumentRevisionQuery) (*DocumentRevisionQuery, error) {
	if p.filter != nil {
		return p.filter(query)
	}
	return query, nil
...
func (p *documentrevisionPager) toCursor(dr *DocumentRevision) Cursor {
	cs_ := make([]any, 0, len(p.order))
	for _, o_ := range p.order {
		cs_ = append(cs_, o_.Field.toCursor(dr).Value)
	}
	return Cursor{ID: dr.ID, Value: cs_}
}

func (p *documentrevisionPager) applyCursors(query *DocumentRevisionQuery, after, before *Cursor) (*DocumentRevisionQuery, error) {
	idDirection := entgql.OrderDirectionAsc
	if p.reverse {
		idDirection = entgql.OrderDirectionDesc
	}
	...
	predicates, err := entgql.MultiCursorsPredicate(after, before, &entgql.MultiCursorsOptions{
		FieldID:     DefaultDocumentRevisionOrder.Field.column,
		DirectionID: idDirection,
		Fields:      fields,
		Directions:  directions,
	})
...
func (p *documentrevisionPager) applyOrder(query *DocumentRevisionQuery) *DocumentRevisionQuery {
	var defaultOrdered bool
	for _, o := range p.order {
		direction := o.Direction
		if p.reverse {
		...
		query = query.Order(o.Field.toTerm(direction.OrderTermOption()))
		if o.Field.column == DefaultDocumentRevisionOrder.Field.column {
			defaultOrdered = true
		}
		if len(query.ctx.Fields) > 0 {
			query.ctx.AppendFieldOnce(o.Field.column)
	...
	if !defaultOrdered {
		...
		query = query.Order(DefaultDocumentRevisionOrder.Field.toTerm(direction.OrderTermOption()))
	}
	return query
}

func (p *documentrevisionPager) orderExpr(query *DocumentRevisionQuery) sql.Querier {
	if len(query.ctx.Fields) > 0 {
		for _, o := range p.order {
			query.ctx.AppendFieldOnce(o.Field.column)
		}
	...
	return sql.ExprFunc(func(b *sql.Builder) {
		...
		b.Ident(DefaultDocumentRevisionOrder.Field.column).Pad().WriteString(string(direction))
	})
}

// Paginate executes the query and returns a relay based cursor connection to DocumentRevision.
func (dr *DocumentRevisionQuery) Paginate(
	ctx context.Context, after *Cursor, first *int,
	before *Cursor, last *int, opts ...DocumentRevisionPaginateOption,
) (*DocumentRevisionConnection, error) {
	if err := validateFirstLast(first, last); err != nil {
		return nil, err
	}
	pager, err := newDocumentRevisionPager(opts, last != nil)
	if err != nil {
		return nil, err
	}
	...
	conn := &DocumentRevisionConnection{Edges: []*DocumentRevisionEdge{}}
	ignoredEdges := !hasCollectedField(ctx, edgesField)
	if hasCollectedField(ctx, totalCountField) || hasCollectedField(ctx, pageInfoField) {
		hasPagination := after != nil || first != nil || before != nil || last != nil
		if hasPagination || ignoredEdges {
...
var (
	// DocumentRevisionOrderFieldCreatedAt orders DocumentRevision by created_at.
	DocumentRevisionOrderFieldCreatedAt = &DocumentRevisionOrderField{
		Value: func(dr *DocumentRevision) (ent.Value, error) {
			return dr.CreatedAt, nil
		},
		column: documentrevision.FieldCreatedAt,
		toTerm: documentrevision.ByCreatedAt,
		toCursor: func(dr *DocumentRevision) Cursor {
			return Cursor{
				ID:    dr.ID,
				Value: dr.CreatedAt,
			}
	...
	// DocumentRevisionOrderFieldUpdatedAt orders DocumentRevision by updated_at.
	DocumentRevisionOrderFieldUpdatedAt = &DocumentRevisionOrderField{
		Value: func(dr *DocumentRevision) (ent.Value, error) {
			return dr.UpdatedAt, nil
		},
		column: documentrevision.FieldUpdatedAt,
		toTerm: documentrevision.ByUpdatedAt,
		toCursor: func(dr *DocumentRevision) Cursor {
			return Cursor{
				ID:    dr.ID,
				Value: dr.UpdatedAt,
	...
	// DocumentRevisionOrderFieldRevision orders DocumentRevision by revision.
	DocumentRevisionOrderFieldRevision = &DocumentRevisionOrderField{
		Value: func(dr *DocumentRevision) (ent.Value, error) {
			return dr.Revision, nil
		},
		column: documentrevision.FieldRevision,
		toTerm: documentrevision.ByRevision,
		toCursor: func(dr *DocumentRevision) Cursor {
			return Cursor{
				ID:    dr.ID,
				Value: dr.Revision,
	...
	// DocumentRevisionOrderFieldStatus orders DocumentRevision by status.
	DocumentRevisionOrderFieldStatus = &DocumentRevisionOrderField{
		Value: func(dr *DocumentRevision) (ent.Value, error) {
			return dr.Status, nil
		},
		column: documentrevision.FieldStatus,
		toTerm: documentrevision.ByStatus,
		toCursor: func(dr *DocumentRevision) Cursor {
			return Cursor{
				ID:    dr.ID,
				Value: dr.Status,
...
// String implement fmt.Stringer interface.
func (f DocumentRevisionOrderField) String() string {
	var str string
	switch f.column {
	case DocumentRevisionOrderFieldCreatedAt.column:
		str = "created_at"
	case DocumentRevisionOrderFieldUpdatedAt.column:
		str = "updated_at"
	case DocumentRevisionOrderFieldRevision.column:
		str = "revision"
	case DocumentRevisionOrderFieldStatus.column:
		str = "STATUS"
	}
	return str
}
...
// MarshalGQL implements graphql.Marshaler interface.
func (f DocumentRevisionOrderField) MarshalGQL(w io.Writer) {
	io.WriteString(w, strconv.Quote(f.String()))
}

// UnmarshalGQL implements graphql.Unmarshaler interface.
func (f *DocumentRevisionOrderField) UnmarshalGQL(v interface{}) error {
	str, ok := v.(string)
	if !ok {
		return fmt.Errorf("DocumentRevisionOrderField %T must be a string", v)
	}
	switch str {
	case "created_at":
		*f = *DocumentRevisionOrderFieldCreatedAt
	case "updated_at":
		*f = *DocumentRevisionOrderFieldUpdatedAt
	case "revision":
		*f = *DocumentRevisionOrderFieldRevision
	case "STATUS":
		*f = *DocumentRevisionOrderFieldStatus
	default:
		return fmt.Errorf("%s is not a valid DocumentRevisionOrderField", str)
	}
	return nil
}

// DocumentRevisionOrderField defines the ordering field of DocumentRevision.
type DocumentRevisionOrderField struct {
	// Value extracts the ordering value from the given DocumentRevision.
	Value    func(*DocumentRevision) (ent.Value, error)
	column   string // field or computed.
	toTerm   func(...sql.OrderTermOption) documentrevision.OrderOption
	toCursor func(*DocumentRevision) Cursor
}

// DocumentRevisionOrder defines the ordering of DocumentRevision.
type DocumentRevisionOrder struct {
	Direction OrderDirection              `json:"direction"`
	Field     *DocumentRevisionOrderField `json:"field"`
}

// DefaultDocumentRevisionOrder is the default ordering of DocumentRevision.
var DefaultDocumentRevisionOrder = &DocumentRevisionOrder{
	Direction: entgql.OrderDirectionAsc,
...
// ToEdge converts DocumentRevision into DocumentRevisionEdge.
func (dr *DocumentRevision) ToEdge(order *DocumentRevisionOrder) *DocumentRevisionEdge {
	if order == nil {
		order = DefaultDocumentRevisionOrder
	}
	return &DocumentRevisionEdge{
		Node:   dr,
		Cursor: order.Field.toCursor(dr),
	}
}

// DocumentRevisionHistoryEdge is the edge representation of DocumentRevisionHistory.
type DocumentRevisionHistoryEdge struct {
	Node   *DocumentRevisionHistory `json:"node"`
	Cursor Cursor                   `json:"cursor"`
}

// DocumentRevisionHistoryConnection is the connection containing edges to DocumentRevisionHistory.
type DocumentRevisionHistoryConnection struct {
	Edges      []*DocumentRevisionHistoryEdge `json:"edges"`
	PageInfo   PageInfo                       `json:"pageInfo"`
	TotalCount int                            `json:"totalCount"`
}

func (c *DocumentRevisionHistoryConnection) build(nodes []*DocumentRevisionHistory, pager *documentrevisionhistoryPager, after *Cursor, first *int, before *Cursor, last *int) {
	c.PageInfo.HasNextPage = before != nil
	c.PageInfo.HasPreviousPage = after != nil
	if first != nil && *first+1 == len(nodes) {
		c.PageInfo.HasNextPage = true
	...
	var nodeAt func(int) *DocumentRevisionHistory
	if last != nil {
		n := len(nodes) - 1
		nodeAt = func(i int) *DocumentRevisionHistory {
			return nodes[n-i]
		}
	} else {
		nodeAt = func(i int) *DocumentRevisionHistory {
	...
	c.Edges = make([]*DocumentRevisionHistoryEdge, len(nodes))
	for i := range nodes {
		node := nodeAt(i)
		c.Edges[i] = &DocumentRevisionHistoryEdge{
			Node:   node,
			Cursor: pager.toCursor(node),
		}
	}
...
// DocumentRevisionHistoryPaginateOption enables pagination customization.
type DocumentRevisionHistoryPaginateOption func(*documentrevisionhistoryPager) error

// WithDocumentRevisionHistoryOrder configures pagination ordering.
func WithDocumentRevisionHistoryOrder(order *DocumentRevisionHistoryOrder) DocumentRevisionHistoryPaginateOption {
	if order == nil {
		order = DefaultDocumentRevisionHistoryOrder
	}
	o := *order
	return func(pager *documentrevisionhistoryPager) error {
		if err := o.Direction.Validate(); err != nil {
			return err
		}
		if o.Field == nil {
			o.Field = DefaultDocumentRevisionHistoryOrder.Field
		}
		pager.order = &o
		return nil
	}
...
// WithDocumentRevisionHistoryFilter configures pagination filter.
func WithDocumentRevisionHistoryFilter(filter func(*DocumentRevisionHistoryQuery) (*DocumentRevisionHistoryQuery, error)) DocumentRevisionHistoryPaginateOption {
	return func(pager *documentrevisionhistoryPager) error {
		if filter == nil {
			return errors.New("DocumentRevisionHistoryQuery filter cannot be nil")
		}
		pager.filter = filter
		return nil
	}
...
type documentrevisionhistoryPager struct {
	reverse bool
	order   *DocumentRevisionHistoryOrder
	filter  func(*DocumentRevisionHistoryQuery) (*DocumentRevisionHistoryQuery, error)
}

func newDocumentRevisionHistoryPager(opts []DocumentRevisionHistoryPaginateOption, reverse bool) (*documentrevisionhistoryPager, error) {
	pager := &documentrevisionhistoryPager{reverse: reverse}
	for _, opt := range opts {
		if err := opt(pager); err != nil {
			return nil, err
		}
	...
	if pager.order == nil {
		pager.order = DefaultDocumentRevisionHistoryOrder
	}
	return pager, nil
}

func (p *documentrevisionhistoryPager) applyFilter(query *DocumentRevisionHistoryQuery) (*DocumentRevisionHistoryQuery, error) {
	if p.filter != nil {
		return p.filter(query)
	}
	return query, nil
...
func (p *documentrevisionhistoryPager) toCursor(drh *DocumentRevisionHistory) Cursor {
	return p.order.Field.toCursor(drh)
}

func (p *documentrevisionhistoryPager) applyCursors(query *DocumentRevisionHistoryQuery, after, before *Cursor) (*DocumentRevisionHistoryQuery, error) {
	direction := p.order.Direction
	if p.reverse {
		direction = direction.Reverse()
	}
	for _, predicate := range entgql.CursorsPredicate(after, before, DefaultDocumentRevisionHistoryOrder.Field.column, p.order.Field.column, direction) {
		query = query.Where(predicate)
	}
	return query, nil
}

func (p *documentrevisionhistoryPager) applyOrder(query *DocumentRevisionHistoryQuery) *DocumentRevisionHistoryQuery {
	direction := p.order.Direction
	if p.reverse {
		direction = direction.Reverse()
	}
	query = query.Order(p.order.Field.toTerm(direction.OrderTermOption()))
	if p.order.Field != DefaultDocumentRevisionHistoryOrder.Field {
		query = query.Order(DefaultDocumentRevisionHistoryOrder.Field.toTerm(direction.OrderTermOption()))
	}
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(p.order.Field.column)
	}
...
func (p *documentrevisionhistoryPager) orderExpr(query *DocumentRevisionHistoryQuery) sql.Querier {
	direction := p.order.Direction
	if p.reverse {
		direction = direction.Reverse()
	}
	...
	return sql.ExprFunc(func(b *sql.Builder) {
		b.Ident(p.order.Field.column).Pad().WriteString(string(direction))
		if p.order.Field != DefaultDocumentRevisionHistoryOrder.Field {
			b.Comma().Ident(DefaultDocumentRevisionHistoryOrder.Field.column).Pad().WriteString(string(direction))
		}
	})
}

// Paginate executes the query and returns a relay based cursor connection to DocumentRevisionHistory.
func (drh *DocumentRevisionHistoryQuery) Paginate(
	ctx context.Context, after *Cursor, first *int,
	before *Cursor, last *int, opts ...DocumentRevisionHistoryPaginateOption,
) (*DocumentRevisionHistoryConnection, error) {
	if err := validateFirstLast(first, last); err != nil {
		return nil, err
	}
	pager, err := newDocumentRevisionHistoryPager(opts, last != nil)
	if err != nil {
		return nil, err
	}
	if drh, err = pager.applyFilter(drh); err != nil {
		return nil, err
	}
	conn := &DocumentRevisionHistoryConnection{Edges: []*DocumentRevisionHistoryEdge{}}
	ignoredEdges := !hasCollectedField(ctx, edgesField)
	if hasCollectedField(ctx, totalCountField) || hasCollectedField(ctx, pageInfoField) {
		hasPagination := after != nil || first != nil || before != nil || last != nil
		if hasPagination || ignoredEdges {
	...
	if drh, err = pager.applyCursors(drh, after, before); err != nil {
		return nil, err
	}
	limit := paginateLimit(first, last)
	if limit != 0 {
	...
	if field := collectedField(ctx, edgesField, nodeField); field != nil {
		if err := drh.collectField(ctx, limit == 1, graphql.GetOperationContext(ctx), *field, []string{edgesField, nodeField}); err != nil {
			return nil, err
		}
	}
	drh = pager.applyOrder(drh)
	nodes, err := drh.All(ctx)
	if err != nil {
		return nil, err
	}
	conn.build(nodes, pager, after, first, before, last)
...
var (
	// DocumentRevisionHistoryOrderFieldCreatedAt orders DocumentRevisionHistory by created_at.
	DocumentRevisionHistoryOrderFieldCreatedAt = &DocumentRevisionHistoryOrderField{
		Value: func(drh *DocumentRevisionHistory) (ent.Value, error) {
			return drh.CreatedAt, nil
		},
		column: documentrevisionhistory.FieldCreatedAt,
		toTerm: documentrevisionhistory.ByCreatedAt,
		toCursor: func(drh *DocumentRevisionHistory) Cursor {
			return Cursor{
				ID:    drh.ID,
				Value: drh.CreatedAt,
			}
	...
	// DocumentRevisionHistoryOrderFieldUpdatedAt orders DocumentRevisionHistory by updated_at.
	DocumentRevisionHistoryOrderFieldUpdatedAt = &DocumentRevisionHistoryOrderField{
		Value: func(drh *DocumentRevisionHistory) (ent.Value, error) {
			return drh.UpdatedAt, nil
		},
		column: documentrevisionhistory.FieldUpdatedAt,
		toTerm: documentrevisionhistory.ByUpdatedAt,
		toCursor: func(drh *DocumentRevisionHistory) Cursor {
			return Cursor{
				ID:    drh.ID,
				Value: drh.UpdatedAt,
	...
	// DocumentRevisionHistoryOrderFieldRevision orders DocumentRevisionHistory by revision.
	DocumentRevisionHistoryOrderFieldRevision = &DocumentRevisionHistoryOrderField{
		Value: func(drh *DocumentRevisionHistory) (ent.Value, error) {
			return drh.Revision, nil
		},
		column: documentrevisionhistory.FieldRevision,
		toTerm: documentrevisionhistory.ByRevision,
		toCursor: func(drh *DocumentRevisionHistory) Cursor {
			return Cursor{
				ID:    drh.ID,
				Value: drh.Revision,
	...
	// DocumentRevisionHistoryOrderFieldStatus orders DocumentRevisionHistory by status.
	DocumentRevisionHistoryOrderFieldStatus = &DocumentRevisionHistoryOrderField{
		Value: func(drh *DocumentRevisionHistory) (ent.Value, error) {
			return drh.Status, nil
		},
		column: documentrevisionhistory.FieldStatus,
		toTerm: documentrevisionhistory.ByStatus,
		toCursor: func(drh *DocumentRevisionHistory) Cursor {
			return Cursor{
				ID:    drh.ID,
				Value: drh.Status,
...
// String implement fmt.Stringer interface.
func (f DocumentRevisionHistoryOrderField) String() string {
	var str string
	switch f.column {
	case DocumentRevisionHistoryOrderFieldCreatedAt.column:
		str = "created_at"
	case DocumentRevisionHistoryOrderFieldUpdatedAt.column:
		str = "updated_at"
	case DocumentRevisionHistoryOrderFieldRevision.column:
		str = "revision"
	case DocumentRevisionHistoryOrderFieldStatus.column:
		str = "STATUS"
	}
	return str
}
...
// MarshalGQL implements graphql.Marshaler interface.
func (f DocumentRevisionHistoryOrderField) MarshalGQL(w io.Writer) {
	io.WriteString(w, strconv.Quote(f.String()))
}

// UnmarshalGQL implements graphql.Unmarshaler interface.
func (f *DocumentRevisionHistoryOrderField) UnmarshalGQL(v interface{}) error {
	str, ok := v.(string)
	if !ok {
		return fmt.Errorf("DocumentRevisionHistoryOrderField %T must be a string", v)
	}
	switch str {
	case "created_at":
		*f = *DocumentRevisionHistoryOrderFieldCreatedAt
	case "updated_at":
		*f = *DocumentRevisionHistoryOrderFieldUpdatedAt
	case "revision":
		*f = *DocumentRevisionHistoryOrderFieldRevision
	case "STATUS":
		*f = *DocumentRevisionHistoryOrderFieldStatus
	default:
		return fmt.Errorf("%s is not a valid DocumentRevisionHistoryOrderField", str)
	}
	return nil
}

// DocumentRevisionHistoryOrderField defines the ordering field of DocumentRevisionHistory.
type DocumentRevisionHistoryOrderField struct {
	// Value extracts the ordering value from the given DocumentRevisionHistory.
	Value    func(*DocumentRevisionHistory) (ent.Value, error)
	column   string // field or computed.
	toTerm   func(...sql.OrderTermOption) documentrevisionhistory.OrderOption
	toCursor func(*DocumentRevisionHistory) Cursor
}

// DocumentRevisionHistoryOrder defines the ordering of DocumentRevisionHistory.
type DocumentRevisionHistoryOrder struct {
	Direction OrderDirection                     `json:"direction"`
	Field     *DocumentRevisionHistoryOrderField `json:"field"`
}

// DefaultDocumentRevisionHistoryOrder is the default ordering of DocumentRevisionHistory.
var DefaultDocumentRevisionHistoryOrder = &DocumentRevisionHistoryOrder{
	Direction: entgql.OrderDirectionAsc,
	Field: &DocumentRevisionHistoryOrderField{
		...
		toTerm: documentrevisionhistory.ByID,
		toCursor: func(drh *DocumentRevisionHistory) Cursor {
			return Cursor{ID: drh.ID}
		},
	},
}

// ToEdge converts DocumentRevisionHistory into DocumentRevisionHistoryEdge.
func (drh *DocumentRevisionHistory) ToEdge(order *DocumentRevisionHistoryOrder) *DocumentRevisionHistoryEdge {
	if order == nil {
		order = DefaultDocumentRevisionHistoryOrder
	}
	return &DocumentRevisionHistoryEdge{
		Node:   drh,
		Cursor: order.Field.toCursor(drh),
	}
}
