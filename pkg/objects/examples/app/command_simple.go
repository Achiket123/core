	...
	return &cli.Command{
		...
		Flags: []cli.Flag{
			&cli.StringFlag{
				...
				Value:   "./tmp/storage",
				Sources: cli.EnvVars("OBJECTS_EXAMPLE_DISK_DIR"),
			},
			&cli.StringFlag{
				Name:    "local-url",
				Usage:   "Local URL used when building presigned links",
				Value:   "http://localhost:8080/files",
				Sources: cli.EnvVars("OBJECTS_EXAMPLE_DISK_URL"),
			},
			&cli.BoolFlag{
				Name:    "keep",
				Usage:   "Keep the storage directory after the example finishes",
				Sources: cli.EnvVars("OBJECTS_EXAMPLE_DISK_KEEP"),
			},
		},
...
func runSimple(ctx context.Context, out io.Writer, cfg simpleConfig) error {
	...
	if cfg.LocalURL == "" {
		cfg.LocalURL = "http://localhost:8080/files"
	}
